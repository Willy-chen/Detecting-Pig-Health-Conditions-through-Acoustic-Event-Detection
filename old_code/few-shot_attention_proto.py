...
class VGGishEncoder(nn.Module):
    def __init__(self, out_dim=64, sample_rate=16000, device=None):
        ...
        # Initialize VGGish model and move to device
        ...
        # Attention mechanism for weighted averaging
    ...
    def _process_batch(self, audio_batch):
        """Process batch audio waveform to VGGish features."""
        batch_size = audio_batch.size(0)
        
        # Single CPU conversion for entire batch
        audio_np_batch = audio_batch.detach().cpu().numpy()
        
        # Collect all examples before GPU transfer
        all_examples = []
        batch_indices = []
        
        for i in range(batch_size):
            try:
                examples = vggish_input.waveform_to_examples(audio_np_batch[i], self.sample_rate)
                if len(examples) > 0:
                    # Check if examples is already a tensor
                    if isinstance(examples, torch.Tensor):
                        examples_tensor = examples.float().to(self.device)
                    else:
                        examples_tensor = torch.from_numpy(examples).float().to(self.device)
                    all_examples.append(examples_tensor)
                    batch_indices.append(i)
                else:
                    # Create dummy example
                    print(f"Warning: No examples generated for batch item {i}, creating dummy example.")
                    dummy_example = torch.zeros((1, 96, 64)).float().to(self.device)
                    all_examples.append(dummy_example)
                    batch_indices.append(i)
            except Exception as e:
                print(f"Error processing batch item {i}: {e}")
                # Create dummy example for failed processing
                dummy_example = torch.zeros((1, 96, 64)).float().to(self.device)
                all_examples.append(dummy_example)
                batch_indices.append(i)
        ...
        # Process with VGGish
        batch_features = []
        for examples_tensor in all_examples:
            with torch.no_grad():  # VGGish is frozen
                features = self.vggish(examples_tensor)  # [N, 128]
    ...
    def forward(self, x):
        ...
        Returns:
            Projected VGGish features of shape [B, out_dim]
        """
        x = x.to(self.device)
        if x.dim() == 3:
            x = x.squeeze(1)
        
        # Efficient VGGish processing
        vggish_features_batch = self._process_batch(x)
        
        # Apply trainable attention and projection
        final_features = []
        for features in vggish_features_batch:
            # Trainable attention mechanism
            attention_scores = self.attention(features).squeeze(-1)  # [N]
            attention_weights = torch.softmax(attention_scores, dim=0)  # [N]
        ...
        batch_features = torch.stack(final_features)  # [B, 128]
        return self.proj(batch_features)  # [B, out_dim] - gradient
