...
def run_evaluation_protocol(X_train_pool, y_train_pool, X_test_final, y_test_final, config, scenario_name, report_path, history_path, is_frame_scenario=False):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_train_pool, y_train_pool), 1):
        ...
        weights = compute_sample_weight('balanced', y=y_train_fold)
        
        # Define eval sets to track performance on both train and validation data
        eval_set = [(X_train_fold, y_train_fold), (X_val_fold, y_val_fold)]
        
        model.fit(X_train_fold, y_train_fold, sample_weight=weights,
                  eval_set=eval_set, verbose=False)
        
        # Retrieve the performance history
        ...
        for i in range(epochs):
            cv_histories.append({
                'fold': fold, 'epoch': i + 1,
                'train_loss': results['validation_0']['mlogloss'][i],
                'train_accuracy': 1 - results['validation_0']['merror'][i],
                'val_loss': results['validation_1']['mlogloss'][i],
                'val_accuracy': 1 - results['validation_1']['merror'][i],
            })
        print(f"Fold {fold} completed. Final Val Accuracy: {cv_histories[-1]['val_accuracy']:.4f}")
    ...
    final_weights = compute_sample_weight('balanced', y=y_train_pool)
    final_model.fit(X_train_pool, y_train_pool, sample_weight=final_weights)

    print("\n-- Evaluating Final Model on Held-Out Test Set --")
    if is_frame_scenario:
        # For frames, we predict then vote
