...
class TrainingParams:
    ...
    num_classes = 50        # Your breathing classes
...
class ESC50Dataset(Dataset):
    def __init__(self, audio_dir, csv_path, folds, params):
        self.audio_dir = audio_dir
        self.params = params
        self.df = pd.read_csv(csv_path)
        # Filter the dataframe to include only the specified folds
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # Pad or trim to ensure a fixed length
            if waveform.shape[1] < self.params.segment_length:
                padding = self.params.segment_length - waveform.shape[1]
                waveform = F.pad(waveform, (0, padding))
            elif waveform.shape[1] > self.params.segment_length:
                waveform = waveform[:, :self.params.segment_length]
            ...
            # Return a dummy tensor on error to prevent crashes
            return torch.zeros(1, self.params.segment_length), 0
...
class KmeansPrototypicalNetworkClassifier(nn.Module):
    ...
    def compute_prototypes(self, embeddings, labels):
        ...
        for class_idx in range(self.num_classes):
            ...
            if class_embeddings.size(0) == 0:
                ...
                continue
            elif class_embeddings.size(0) == 1:
                # Only one example, use it as the single prototype
                prototypes[class_idx, 0] = class_embeddings[0]
    ...
    def compute_distances(self, query_embeddings, prototypes, prototype_valid):
        ...
        min_distances = torch.full((batch_size, self.num_classes), float('inf'), device=query_embeddings.device)
        
        for class_idx in range(self.num_classes):
            valid_prototypes = prototypes[class_idx][prototype_valid[class_idx]]
            
            if valid_prototypes.size(0) == 0:
                # No valid prototypes for this class, use large distance
                continue
            
            # Compute distances to all valid prototypes for this class
            if self.distance_metric == 'euclidean':
                distances = torch.cdist(query_embeddings, valid_prototypes, p=2)
            elif self.distance_metric == 'cosine':
                query_norm = F.normalize(query_embeddings, p=2, dim=1)
                proto_norm = F.normalize(valid_prototypes, p=2, dim=1)
                similarities = torch.mm(query_norm, proto_norm.t())
                distances = 1 - similarities
            elif self.distance_metric == 'squared_euclidean':
                distances = torch.cdist(query_embeddings, valid_prototypes, p=2) ** 2
            else:
                raise ValueError(f"Unknown distance metric: {self.distance_metric}")
            
            # Take minimum distance to any prototype of this class
            min_distances[:, class_idx] = distances.min(dim=1)[0]
...
def evaluate_model(loader, model, criterion, target_names, fold):
    model.eval()
    all_labels, all_preds = [], []
    with torch.no_grad():
        for inputs, labels in tqdm(loader, desc=f"Evaluating Fold {fold}"):
    ...
    # Save report to file
    with open(f"fold_{fold}_report.txt", "w") as f:
        f.write(report_str)
...
if __name__ == "__main__":
    ...
    target_names = sorted(full_df['category'].unique())
    
    all_folds = [1, 2, 3, 4, 5]
    fold_accuracies = []
    
    for test_fold in all_folds:
        print(f"\n===== Starting Cross-Validation: Fold {test_fold} as Test Set =====")
        
        # Split folds for training, validation, and testing
        train_folds = [f for f in all_folds if f != test_fold]
        val_fold = train_folds.pop() # Use one of the training folds for validation
        
        # Create datasets
        train_set = ESC50Dataset(audio_dir, csv_path, folds=train_folds, params=params)
        val_set = ESC50Dataset(audio_dir, csv_path, folds=[val_fold], params=params)
        test_set = ESC50Dataset(audio_dir, csv_path, folds=[test_fold], params=params)
        ...
        accuracy = evaluate_model(test_loader, model, nn.CrossEntropyLoss(), target_names, test_fold)
